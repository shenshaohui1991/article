...
;(function() {
  ...
  /** Object.toString.call(this)方法的返回结果，用于判断类型 */
  ...
  var reEscapedHtml = /&(?:amp|lt|gt|quot|#39|#96);/g,
      reUnescapedHtml = /[&<>"'`]/g,
      /** RegExp.prototype.source 返回 正则表达式的内容 */
  ...
  function runInContext(context) {
     ...
     * @param {boolean} [predicate=isFlattenable] 迭代的前置条件
    ...
    function baseSlice(array, start, end) {
      ...
      if (start < 0) {
        start = -start > length ? 0 : (length + start); // 转化成正数
      }
    ...
    function copyArray(source, array) {
      ...
      array || (array = Array(length)); // array不存在，则初始化
      while (++index < length) {
        array[index] = source[index]; // array可以指定length，也可以不指定，同样的用法
      }
      return array;
    }
     ...
     * @param {number} [size=1] 切分的大小 默认 1
     * @param- {Object} [guard] 是否可以使用类似`_.map`的方法迭代器
     * @returns {Array} Returns 新的数组
     * @example
     *
     * _.chunk(['a', 'b', 'c', 'd'], 2);
     * // => [['a', 'b'], ['c', 'd']]
    ...
    function concat() {
      ...
      if (length < 2) {
        return length ? copyArray(array) : []; // 浅复制
      }
     ...
     */
    function castArray() {
      if (!arguments.length) {// 无参数，则返回空数组
        return [];
      }
